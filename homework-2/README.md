## Итоги третьего и четвертого дней

### Домашнее задание
* [Задача 1: улучшаем поиск Яндекса](hw-1.md)
* [Задача 2: работаем с API Яндекс.Переводчика](hw-2.md)
* [Задача 3: пишем TodoApp](hw-3.md)

### Полезные ссылки

#### learn.javascript.ru
1. [Документ и объекты страницы](http://learn.javascript.ru/document)
1. [Основы работы с событиями](http://learn.javascript.ru/events-and-interfaces)
1. [События в деталях](http://learn.javascript.ru/event-details)
1. [Формы, элементы управления](http://learn.javascript.ru/forms-controls)

#### Frontender Magazine
1. [Что такое DOM?](http://frontender.info/dom/)
1. [Введение в события DOM](http://frontender.info/an-introduction-to-dom-events/)

#### Прочее
1. [Список всех DOM-событий на Mozilla Developer Network](https://developer.mozilla.org/en-US/docs/Web/Events)
